...
def loadMatrix():
    csv_file = '../DataAnalysis/s4_ipsi_nnls_171203.csv'
    df = pd.read_csv(csv_file, index_col=0, header=0)

    fffb_file = '../DataAnalysis/fffb_dawd.csv'
    ...
    df.to_csv('../Release/conn_matrix.csv', header=True, index=True)

    return acronym_list


def hierarchy(acronym_list):
    csv_file = '../DataAnalysis/s2_171020.csv'
    df = pd.read_csv(csv_file, index_col=0, header=0)
    ...
    csv_file1 = '../DataAnalysis/finale_matrix.csv'
    ...
    df1.to_csv('../Release/port_size_matrix.csv', index=0)

    with open('../Release/hierarchy.txt', 'w') as output_file:
        with open('../Release/port_size_matrix.csv', 'r') as input_file:
            [output_file.write('\t'.join(row) + '\n') for row in csv.reader(input_file)]
        # output_file.close()

    df1.to_csv('../Release/port_size_matrix.csv', header=True, index=True)

    return hierarchy_list


def extractMatrix(hierarchy_list):
    csv_file = '../Release/conn_matrix.csv'
    df = pd.read_csv(csv_file, index_col=0, header=0)
    ...
    df3.to_csv('../Release/conn_matrix.csv', header=True, index=True)
...
def connectivity():
    ...
    with open('../Release/connection.txt', 'w') as my_output_file:
...
def blockDiagram():
    ...
    with open('../Release/blockDiagram.txt', 'w') as f:
        for idx in df.index:
...
def region(acronym_list):
    csv_file = '../DataAnalysis/ontology_170731.csv'
    df = pd.read_csv(csv_file, index_col=0, header=0)
    ...
    region_list = []
    with open('../Release/regions.txt', 'w') as f:
